...
def crop_faces(folder_path, remove_bg = True, upscale = 1, fidelity_weight=0.5, get_mask = False):
    ...
    model_face_restorer = CodeFormerRestorer(
                                        ...
                                        face_upsample = True, 
                                        bg_tile = 400, 
                                        detection_model = 'retinaface_resnet50',  # Option: YOLOv5n (dont't forget to change in config.py)
    ...
    for file_name in os.listdir(folder_path):
        if file_name.lower().endswith(ACCEPTED_FILE_EXTENSIONS):
            ...
            if len(filtered_idxs) > 0:
                    ...
                    if len(results[0].boxes.xyxy) > 0:
                        ...
                        if restored_img is not None:
                            ...
                            if remove_bg:
                                final_image_en.save(f"output/res_Face_noBG_{file_name}")
                            else:
                                final_image_en.save(f"output/res_Face_BG_{file_name}")
                        else:
...
if __name__ == "__main__":
    image_folder = "images/all"
    crop_faces(image_folder, 
               remove_bg = False, 
               upscale = 1,
               fidelity_weight=0.5,
               get_mask = True,
